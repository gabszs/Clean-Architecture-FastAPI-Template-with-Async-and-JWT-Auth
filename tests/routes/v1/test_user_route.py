...
async def test_get_all_users_should_return_200_OK_GET(
    ...
    assert response_json["search_metadata"] == default_username_search_options | {"total_count": expected_lenght}
    assert all(
        [
...
async def test_get_all_users_with_page_size_should_return_200_OK_GET(
    ...
    assert response_json["search_metadata"] == query_find_parameters | {"total_count": 5}
...
async def test_get_all_users_with_pagination_should_return_200_OK_GET(
    ...
    assert response_json["search_metadata"] == query_find_parameters | {
...
async def test_disable_user_should_return_200_OK_DELETE(session, client, normal_user, moderator_user_token):
    ...
    response = await client.delete(f"{base_users_url}/disable/{normal_user.id}", headers=token)
    response_json = response.json()
